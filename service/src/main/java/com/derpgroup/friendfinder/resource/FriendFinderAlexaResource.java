...
package com.derpgroup.friendfinder.resource;
...
import com.derpgroup.friendfinder.FriendFinderMetadata;
import com.derpgroup.friendfinder.MixInModule;
import com.derpgroup.friendfinder.configuration.MainConfig;
import com.derpgroup.friendfinder.manager.FriendFinderManager;
 ...
 * @since 0.0.1
 */
@Path("/friendfinder/alexa")
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class FriendFinderAlexaResource {

  private static final Logger LOG = LoggerFactory.getLogger(FriendFinderAlexaResource.class);
  
  private static final List<String> UNSUPPORTED_SSML_TAGS = Collections.unmodifiableList(Arrays.asList(
      "emphasis"
      ));

  private FriendFinderManager manager;
  
  public FriendFinderAlexaResource(MainConfig config, Environment env) {
    manager = new FriendFinderManager();
  }
  ...
  public SpeechletResponseEnvelope doAlexaRequest(SpeechletRequestEnvelope request, @HeaderParam("SignatureCertChainUrl") String signatureCertChainUrl, 
    ...
    FriendFinderMetadata metadata = null;
    try {
      ...
      metadata = mapper.convertValue(sessionAttributes, new TypeReference<FriendFinderMetadata>(){});
      ...
      card.setContent(builder.getFullTextMessage()!=null?builder.getFullTextMessage():builder.getRawText());
      card.setTitle(builder.getShortFormTextMessage()!=null?builder.getShortFormTextMessage():"FriendFinder");
